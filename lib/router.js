...
proto.applyRoutes = function applyRoutes(routes) {
  ...
  for (actionName in routes) {
    ...
    if (Array.isArray(action)) {
      action.forEach(_self.applyRoute.bind(_self, actionName));
    } else {
...
proto.applyRoute = function applyRoute(actionName, action) {
  ...
  if (typeof action === 'function') {
    ...
    throw new Error('Wrong definition of the route. It must be a function or an object with `fn` property.');
  }
};
